...
**setup-from-scratch.sh** installs and configures a full Matrix stack on a fresh Debian or Ubuntu server. It prompts for your domain, server name, and options, then installs and configures everything we use: Synapse, Postgres, nginx, TLS (Let's Encrypt), coturn, optional Prometheus/Grafana, fail2ban, and the backup script.
...
- **Repo:** Copy this repo to the server so the script can find config templates (synapse yamls, nginx snippets, fail2ban, grafana, backup script). Example: `scp -r hardened-matrix-stack-install-from-scratch user@server:/tmp/`
...
5. **Monitoring (optional):** Prometheus + node exporter, Grafana; Prometheus scrape config and alerts; Grafana provisioning (dashboard + datasource).
...
Element Call, Mjolnir, Maubot, and Discord bridge are **not** installed by this script; deploy them separately using the README and scripts in this repo.
